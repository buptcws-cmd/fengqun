...
- [Full-Gate And Review Reuse](#full-gate-and-review-reuse)
...
## Full-Gate And Review Reuse

Reuse a completed full validation gate or formal review only for an exact immutable key containing:

- `subject_hash`: the complete frozen candidate/evidence subject, not a branch name or mutable path;
- every applicable `gate_hash` and `charter_hash`: the exact gate definition, reviewer charter, governing instructions, and acceptance policy; if both apply, bind both;
- `runtime_identity`: the exact execution/runtime environment identity required by the gate;
- `toolchain_identity`: the exact validator, build, test, package, or review toolchain identity required by the gate;
- `outcome_lock_identity`: the exact outcome-lock revision plus a content digest or other immutable identity.

Treat any missing, `UNKNOWN`, conflicting, or changed component as a cache miss and rerun the full gate/review. Candidate revision alone, a literal old `review passed`, or matching human-readable labels never authorize reuse. Archive relocation or index-only metadata outside the subject does not invalidate reuse only when the entire key remains identical and the referenced evidence is still verifiable.

Subsystem evidence composes with this key; it does not take precedence over it. For example, an OpenSpec review artifact may bind its exact subject paths and Git blob OIDs, but 野蜂 total-control must still bind the applicable gate/reviewer charter, runtime, toolchain, and outcome-lock identities in the same reuse receipt. A subsystem field marked `not-applicable` is accepted only when the subsystem contract records why that component does not apply and 野蜂 is not currently governing it. While 野蜂 is active, none of its applicable key components may be omitted or replaced with `not-applicable`.
