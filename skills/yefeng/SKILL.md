...
1. reconcile hot truth and read event/message deltas after recorded cursors;
2. verify startup level, epoch, assignments, leases, process budget, broker, and repository identities;
3. ingest completed runs and accepted transport facts, then promote only verified facts into tracked control truth;
4. route messages, resolve cleared blockers, and resume/launch every safe capacity-allowed role;
5. inspect exact evidence and integrate only review-ready checkpoints;
6. update machine state and concise human views under the writer fence;
7. clean exactly matched terminal processes and disposition completed evidence;
8. repeat until quiescent, then report the next executable action or concrete blocker.
...
Track compact delivery signals per cycle: merged checkpoints, product-path-closed checkpoints, candidate attempts, review rounds, full build/package rounds, active-state size, running-role count, and cleanup debt. Record token usage only when exposed by the host; never estimate consumed tokens as fact.

## Evidence Retention

Runtime-heavy logs stay ignored by default. After a reviewed batch reaches a terminal control disposition, use the dry-run, committed `PREPARED`, exact-leaf Apply protocol in [`references/run-evidence-retention.md`](references/run-evidence-retention.md). Missing bindings protect legacy runs. Never recursively delete run directories or weaken recovery/review evidence to save space.
